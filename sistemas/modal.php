...
if ($_POST['action'] == 'searchCliente') {
  if (!empty($_POST['cliente'])) {
    ...
    $query = mysqli_query($conexion, "SELECT * FROM cliente WHERE identificacion LIKE '$identificacion'");
    mysqli_close($conexion);
    $result = mysqli_num_rows($query);
    $data = '';
    if ($result > 0) {
...
if ($_POST['action'] == 'addCliente') {
  ...
  $query_insert = mysqli_query($conexion, "INSERT INTO cliente(identificacion, nombre, telefono, direccion, usuario_id) VALUES ('$identificacion','$nomnre','$telefono','$direccion','$usuario_id')");
  if ($query_insert) {
